...
## [411] 2026-09-17 - 手工绑定人工事件房门与调试信息面板独立关闭

**用户指令**：
> 看起来还是有问题，我们还是手动去做吧。1.在调试面板里面，我会点击跳转到下一个事件，然后呢我去点击对应的门。把它绑定到上面去。2.如果已经绑定了个门，他可以去点击另外一个门覆盖它的绑定。如果一个门都不用绑定，那它就不显示。那点击这个时候呢，我们每一个房间其实都是白色的字。如果绑定了那就把那个房间写上一个。右边的小锁（或者其他简易的图标）的一个样子。3.不过有个很大的问题是，点击调试面板之后，面板太大了会挡住，导致我没办法操作下面的东西。所以，我们现在对调试面板做一个改动，信息面板可以右上角有个叉，单独被关闭，但是下一次再点击调试面板的时候，用默认把它打开。

**实现方案**：

* **人工事件房门真值**：`MarkedEvent` 新增可选 `portalRoomId` 并持久化；“跳转到下一个事件”同时把该事件设为当前绑定目标，点击另一房门即可覆盖原绑定，未绑定事件不显示锁。
* **绑定可视化**：新增 `ManualEventPortalBindingView`，已绑定房间的白色名称右侧绘制小锁；点击 Portal 多边形或房间名称附近均可完成绑定，绑定/改绑后立即刷新并给出短 Toast。
* **调试面板独立关闭**：调试信息面板右上角新增 X，只隐藏右半屏信息面板而不退出调试模式；关闭后仍可操作下面的房门，再次点击“调试面板”时优先恢复信息面板。
* **诊断真值接入**：`EventDiagnosticMatcher` 与诊断 JSON 使用人工 `portalRoomId` 作为门级 GT，可区分 MATCH / WRONG_PORTAL；未绑定事件仍不伪造门级真值。
* **验证与收尾**：临时 CI 最终 `testDebugUnitTest + assembleDebug` 已通过；按用户要求后续不保留云端编译 workflow，由本地负责正常编译。

---
## [410] 2026-09-17 - 人工事件自动推断真实房门并可视化核对
...
---
## [406] 2026-09-16 23:27:00 - V4.1 调试层改为逐像素真实 Mask
...
* **只改调试视觉**：不修改门开关、灯光处理、进出判定或人数账本；当前任务只把可视化从轮廓填充改为真实二值像素。
* **三层真实 Mask**：黄色显示相邻帧阈值后的 MotionMask；橙色显示当前帧相对事件固定参考图、形态学处理前的原始背景差；青色显示当前 HumanOwnedMask；B 版绿色光流线继续保留。
* **禁止轮廓填充**：删除 `findContours(RETR_EXTERNAL)` + `Path.fill` 调试路径；新增 `GateMaskDebug`，把每行连续非零像素压缩成水平运行段，零像素洞原样保留，Canvas 关闭抗锯齿后逐段画回源画面坐标。
* **避免陈旧画面**：ARMED/OFF 不做像素处理时清理旧门 mask；视觉异常、几何重建和 close 时同步清理调试缓存。
* **调试面板**：新增动态/背景差/人体三类像素计数和颜色图例。
* **验证**：完整 `:app:testDebugUnitTest` 与 `:app:assembleDebug` 通过；新增 2 条 GateMaskDebug 单测，验证零像素空洞不会被填充、空 mask 不产生绘制段。Debug APK `128269700` bytes，SHA256 `b20d4b3c13a6bd30443669480c8af265b689579c88277445ae6e497a31ecba69`。
* **边界**：如果新 APK 中肉眼不变的区域仍出现黄色/橙色，这时已不是轮廓绘制造成，而是该像素真实超过当前阈值（默认 18）；后续再据此调整阈值/噪声模型。开关门、开关灯本轮明确未处理。

---
## [405] 2026-09-16 19:45:00 - V4.1 Event ROI 门口事件视觉重构
...
* **基线保留**：以用户本地提交 `912131e`（房间算法日志合并到调试面板）为新基线，完整保留该 UI/日志改动。
* **事件调度**：新增 `GateActivityScheduler`，门状态改为 OFF / ARMED / ACTIVE / HOLD；未正式锁定但达到较低 person 分数的 YOLO/Tracker 候选可以提前 ARMED，只有下半身/脚点真实靠近门才 ACTIVE，候选本身不能修改人数。
* **只处理相关门**：默认最多同时运行 2 扇 ACTIVE/HOLD 门；无人在门口时差分、MOG2、LK 全部停止。所有门只以约 10fps 保存约 1 秒局部灰度 pre-roll，不形成待算工作队列。
* **高分辨率局部图**：删除旧 `GateVision.kt` / `GateLk.kt` 的整帧 `480/640` 工作图路径；默认 `captureEdge=2560`，当前约 2232 宽视频不因 Portal 再缩小，直接从源截图按每扇 Portal 裁高分辨率 ROI。
* **三方案同步重构**：A 只在活跃门做局部固定参考差分；B 只在活跃门裁块做 15×15 正反向 LK、默认 128 点并支持单门短历史逆向来源核验；C 只在事件开始后用该门 pre-roll 预热 OpenCV MOG2，事件期间冻结学习。
* **出门回溯**：严格 Tracker 通常约 0.5 秒后锁定人体时，只回溯当前几何候选的一扇门最近约 1 秒历史；A/C 看局部前景由空到有人，B 可追加逆向 LK，不回放其他门，更不补算整段历史队列。
* **差分精度/显示**：取消旧 4×8 黄色大砖块显示，真实前景在源分辨率门 ROI 内像素级计算；调试层直接画 `findContours` 得到的黄色半透明真实轮廓。内部 ownership 覆盖改为更细的 8×12 统计，HOLD 膨胀降为 3×3。
* **全局保护**：整图灰度/整图 absdiff 已移除，只保留 24 个稀疏亮度采样点检测整体曝光/机位突变。
* **设置迁移**：新增历史长度、HOLD、候选分数、靠门距离、裁块 padding、最大活跃门、轮廓面积等参数；旧 V4 保存配置首次读取时不再沿用原整图缩放语义。
* **验证**：`:app:testDebugUnitTest` 与 `:app:assembleDebug` 通过；87 项单测，失败 0、错误 0、跳过 0。Debug APK `128269700` bytes，SHA256 `30856028131dc517ad0010663ddbed1a7238bc58e3da38f21f480f6e10c64fca`，仅 `arm64-v8a`；YOLO26 detection / pose w8a32 资产 SHA256 保持 `7a598838082251ef8e1d3b8f06f356ad532f8fe3a24c7eeeed68b52d28525036` / `8409d9109f1bb374e28a9ff925205580e62cbcf73528b11777b667f8e28611f8`。
* **边界**：本轮验证证明编译、单测和 APK 完整性，不声称真机已达到 10/20fps，也不声称真实测试视频准确率；面板已显示活跃门数、历史 KB、Pose/视觉/整轮耗时用于实测。

---

## [404] 2026-09-16 09:02:00 - 房间算法日志合并到调试面板

**用户指令**：
> 左上角那个 log 窗太脑残了，合并到调试面板里面去。

**实现方案**：

* 移除 V3/V4 画面左上角的独立黑色文本窗，保留门线、特征点、人体状态等几何标注。
* 将算法名称、人数、帧率、Pose/视觉/整轮耗时、截图耗时、跳采和异常状态改为结构化面板快照。
* 看人调试面板与长按复制共用同一份实时数据；调试面板关闭时不再显示算法文本。

---

## [403] 2026-09-16 00:50:07 - 同步并部署 Portal V3 光流房间算法
...
*   **变更摘要**
    *   任务目的：同步云端 Portal V3 人体光流门控与占用算法，并完成本机测试、构建、安装和启动验证。
    *   修改文件：从云端同步 Portal V3 Core/Vision/Overlay/Settings/Types/FrameHub、Registry、Pose/Video/MainActivity 接入、设置页、测试与四份算法文档；本地合并补充历史记录。
    *   涉及方法：正反向 LK 光流、有限门线跨越、相邻门竞争、遮挡待定、逆向回溯和独立人数账本。
    *   同步结果：当前分支由 `5b9b983` 快进至 `9692e09`。
    *   验证结果：`:app:testDebugUnitTest`、`:app:assembleDebug`、`:app:installDebug` 全部成功；APK 已安装到 `PKB110` 并冷启动成功。YOLO26 检测与 Pose 均继续使用 GPU，应用无启动崩溃。

---

## [396] 2026-09-15 19:45:01 UTC - 新增三种可切换 V4 门口算法及参数设置
...
* **变更摘要**
  ...
  * 验证：Android 单元测试 82 项通过，失败 0、错误 0、跳过 0；:app:assembleDebug 通过；实际 OpenCV 4.13 AAR 含 MOG2/Video 类与 arm64 原生库。
  * 安装包：128253316 字节；相比 V3 +81979 字节；SHA256 54d2d70a667b11af4905f56e566096222d3e0e7f4e8433826edc1b7b9ec172c6；ABI 仅 arm64-v8a。
  * 限制：没有真机或用户视频验证；差分/C 不伪造出门早期人体轨迹，B 有界逆向回看仅为来源辅助；隐藏初始人数须由已有初值设置说明；没有足够可见证据时不能保证每次唯一判定。

---

## [395] 2026-09-16 00:25:20 - 新增可切换 Portal V3 人体光流门槛算法
...
* **变更摘要**
  ...
  * 补充安全检查：本地/远端身份切换保留匿名人数名额而非清空账本；中途启动不误用视频零点初值；新增相应回归测试。
  * 学习边界：仅从明确测得的跨门轨迹保存最近十二次方向弱先验；未声称实现完整遮挡深度学习或少样本必然收敛。
  * 验证：现有及新增单元测试合计 46 项通过，其中 PortalV3CoreTest 32 项；:app:assembleDebug 通过；仅 arm64-v8a；两套 w8a32 资产 SHA256 与改造前一致。
  * 安装包：128171337 字节，较前版变化 +114688 字节；SHA256 15340432e8b5608777af9709bdd7a306115feec68d20d19dcb9cc95c5dc661a2。
...
**用户指令**：
> 手部帧率正常，但快速移动时会丢失并在稍后重新跟上；修复该问题。

**实现方案 (Implementation)**：

*   **变更摘要**
    *   任务目的：避免快速运动产生的瞬时异常点集立即重建模型，消除约数百毫秒的跟踪断档。
    *   修改文件：`HandSmokeTester.kt`、`AppSettings.kt`、codexHistory.md、dialogueHistory.md。
    *   涉及方法：`HandSmokeTester.processResult`、`AppSettings.init`。
    *   关键改动：
      *   单帧退化结果仅丢弃并保留上一有效手，连续退化达到 1000ms 后才重建 Hand Landmarker。
      *   任一有效手重新出现时立即清除连续退化计时。
      *   对历史保存的检测阈值与存在阈值同时为 0.00 的异常配置执行一次迁移，恢复到稳定基线 0.50。
      *   设置项仍保持可调，不改变后续用户主动调参能力。
