 ...
 * Portal 专用短时视觉跟踪实验实现。
 *
 * 使用 OpenCV 主仓库 video 模块里的 TrackerMIL，不依赖 opencv_contrib。
 * 只负责“同一视觉目标当前在哪里”，不负责房间切换，也不把 tracker 返回值解释为人体置信度。
 */
class MilPortalVisualTracker : PortalVisualTracker {
    override val trackerId: String = PortalVisualTrackerRegistry.OPENCV_MIL_ID
    ...
    override fun track(input: PortalVisualTrackerFrameInput): List<PortalVisualTrack> {
        // Portal episode 未启动时不做 OpenCV 初始化/Bitmap 转换，避免全程白跑视觉跟踪前处理。
        ...
        try {
            ...
            // YOLO 重新稳定看到同一 ID 时，周期性用检测框重新锚定，限制 MIL 长期漂移。
            if (detectorPose != null && detectorPose.isConfirmed &&
            ...
            Log.w(TAG, "MIL update failed: ${t.message}", t)
            clearActive()
            return emptyList()
        } finally {
            rgb.release()
    ...
    private fun initializeFromPose(
        ...
        return try {
            ...
            Log.w(TAG, "MIL init failed: ${t.message}", t)
            clearActive()
            false
        }
    }
    ...
    companion object {
        private const val TAG = "PortalVisualMIL"
